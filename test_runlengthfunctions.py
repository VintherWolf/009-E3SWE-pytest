...
if __name__ == '__main__':
    import sys
    from src import runlengthfunctions
    ...
    if len(sys.argv) == 1:  # if not using CLI but user input
        choice = input(
            "Do you want to perform fuzzing tests? (y/yes or any other for no)\n")
        ...
        choice = input(
            "Do you want to perform hypothesis tests? (y/yes or any other for no)\n")
        if choice == 'y' or choice == 'yes':
            hypoTest()
        else:
